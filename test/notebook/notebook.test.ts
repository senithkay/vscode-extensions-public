...
import { MIME_TYPE_JSON, MIME_TYPE_TABLE } from "../../src/notebook";
import { ExtendedLangClient } from "../../src/core/extended-language-client";
import { getServerOptions } from "../../src/server/server";
import { getBallerinaCmd } from "../test-util";
import { assertContainsMimeTypes, assertHasTextOutputInVSCode, closeActiveWindows, 
    openNotebook, saveActiveNotebook } from "./utils";

const PROJECT_ROOT = path.join(__dirname, '..', '..', '..');
const DATA_ROOT = path.join(PROJECT_ROOT, 'test', 'data');
const sampleBalnotebook = path.join(DATA_ROOT, "sample.balnotebook");

suite("Ballerina Notebook Saving Tests", function () {
    this.timeout(20000);
    ...
    test("Verify output & metadata when re-opening", async (done) => {
...
suite("Ballerina Notebook Execution", function () {
    this.timeout(20000);
    let langClient: ExtendedLangClient;
    ...
    test("Verify code execution", async (done)  => {
        const balnotebook = await openNotebook(sampleBalnotebook);
        // TODO: create a notebook from scratch and add cell contents
        // with proposed API - vscode.proposed.notebookEditorEdit.d.ts
