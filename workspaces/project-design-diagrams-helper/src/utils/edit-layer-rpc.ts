...
import {
    BallerinaTriggerResponse, BallerinaTriggersResponse, CMLocation as Location, CMAnnotation as Annotation, GetComponentModelResponse
...
import { deleteProjectComponent, getActiveChoreoProject, getComponentModel, go2source, showChoreoProjectOverview } from "./shared-utils";
...
export class EditLayerRPC {
    ...
    constructor(webview: WebviewPanel, langClient: any, context: ExtensionContext, isChoreoProject: boolean, activeChoreoProject: Project | undefined) {
        ...
        this._messenger.onRequest({ method: 'getComponentModel' }, async (): Promise<GetComponentModelResponse> => {
            return getComponentModel(langClient, isChoreoProject);
        });

        this._messenger.onRequest({ method: 'showChoreoProjectOverview' }, async (): Promise<void> => {
        ...
        this._messenger.onRequest({ method: 'deleteComponent' }, async (args: any[]): Promise<void> => {
            if (isChoreoProject && !activeChoreoProject) {
                activeChoreoProject = await getActiveChoreoProject();
            }
    ...
    static create(webview: WebviewPanel, langClient: any, context: ExtensionContext, isChoreoProject: boolean, activeChoreoProject: Project | undefined) {
        return new EditLayerRPC(webview, langClient, context, isChoreoProject, activeChoreoProject);
    }
}
