...
interface GraphqlServiceFormProps {
    model?: ServiceDeclaration | ModulePart;
}

const GRAPHQL_MODULE_QUALIFIER = 'graphql';
const GRAPHQL_IMPORT = new Set<string>(['ballerina/graphql']);

export function GraphqlServiceForm(props: GraphqlServiceFormProps) {
    ...
    const serviceParamChange = async (servicePath: string, port: string, name: string, fieldType?: string) => {
        ...
        if (!partialST.syntaxDiagnostics.length) {
            ...
            onChange(updatedContent, partialST, GRAPHQL_IMPORT, currentModel, currentValue, [], offset);
        } else {
    ...
    const getListenerSelector = () => (
        <>
            <FormHelperText className={formClasses.inputLabelForRequired}>
                <FormattedMessage
                    id="lowcode.develop.connectorForms.GRAPHQL.selectlListener"
