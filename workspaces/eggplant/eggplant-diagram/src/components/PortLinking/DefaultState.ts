	...
	constructor() {
		...
		this.registerAction(
			new Action({
				...
				fire: (eventInput) => {
                    let event = eventInput as ActionEvent<MouseEvent>;
					...
					else if (element instanceof WorkerPortModel) {
						return;
					}
		...
		this.registerAction(
			new Action({
				...
				fire: (eventInput) => {
                    let event = eventInput as ActionEvent<TouchEvent>;
		...
		this.registerAction(
			new Action({
				...
				fire: (eventInput) => {
					...
					if (element instanceof WorkerPortModel) this.transitionWithEvent(this.createLink, event);
				}
			})
		);
	}
