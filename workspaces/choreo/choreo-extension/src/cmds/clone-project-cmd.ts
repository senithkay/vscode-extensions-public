 ...
 */
import { ExtensionContext, ProgressLocation, QuickPickItem, QuickPickItemKind, Uri, commands, window } from "vscode";
import { CommandIds, ComponentKind, GitProvider } from "@wso2-enterprise/choreo-core";
                ...
                if (userInfo) {
                    ...
                    if (selectedProject.gitProvider && selectedProject.gitOrganization && selectedProject.repository) {
                        ...
                        updateContextFile(
                            ...
                            authStore.getState().state.userInfo!,
                            selectedProject,
                            selectedOrg,
                            projectCache
                        );
                        ...
                        const workspaceFilePath = createWorkspaceFile(
                            ...
                            components?.map((item) => ({
                                ...
                                fsPath: join(
                                    clonedResp[0].clonedPath,
                                    item.spec.source.github?.path || item.spec.source.bitbucket?.path || ""
                                ),
                            })) ?? []
                        );
                        ...
                        if (repoSet.size > 1) {
                            ...
                            if (selection?.label === "Clone entire project") {
                                ...
                                repoSet.clear();
                                repoSet.add(
                                    (selection as any)?.item.spec.source.github?.repository ||
                                        (selection as any)?.item.spec.source.bitbucket?.repository
                                );
                            } else {
                                throw new Error(
                                    "Repository or component selection is required in order to clone the repository"
                                );
                            }
                        }
                        ...
                        if (repoSet.size === 1) {
                            ...
                            const { dirPath: projectDirPath } = createDirectory(
                            ...
                            const clonedResp = await cloneRepositoryWithProgress(
                                ...
                                Array.from(repoSet).map((selectedRepoUrl) => {
                                    ...
                                    const matchingComp = components?.find(
                                        (item) =>
                                            selectedRepoUrl ===
                                            (item.spec.source.github?.repository ||
                                                item.spec.source.bitbucket?.repository)
                                    );
                            ...
                            for (const clonedRespItem of clonedResp) {
                                ...
                                for (const item of components) {
                                    const componentRepo = parseGitURL(
                                        item.spec.source.github?.repository || item.spec.source.bitbucket?.repository
                                    );
