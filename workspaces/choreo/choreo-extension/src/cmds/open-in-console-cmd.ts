 ...
 */
import { ExtensionContext, commands, env, Uri, window, QuickPickItem, QuickPickItemKind } from "vscode";
import { CommandIds, ComponentKind, Organization, Project } from "@wso2-enterprise/choreo-core";
import { selectOrg, selectProject, getUserInfoForCmd, quickPickWithLoader } from "./cmd-utils";
            ...
            async (params: { organization: Organization; project: Project; component: ComponentKind }) => {
                try {
                    ...
                    if (userInfo) {
                        ...
                        let projectBaseUrl = `${choreoEnvConfig.getConsoleUrl()}/organizations/${
                            selectedOrg?.handle
                        }/projects/${selectedProject.id}`;

                        if (params?.component) {
                            env.openExternal(
                                Uri.parse(`${projectBaseUrl}/components/${params?.component.metadata.handler}`)
                            );
                            ...
                            if (cacheComponentPick.length > 0) {
                                cacheQuickPicks.push(
                                    { kind: QuickPickItemKind.Separator, label: "Components" },
                                    ...cacheComponentPick
                                );
                            }
                            ...
                            const selectedOption = await quickPickWithLoader({
                                ...
                                loadQuickPicks: async () => {
                                    ...
                                    dataCacheStore
                                        .getState()
                                        .setComponents(selectedOrg.handle, selectedProject.handler, components);

                                    const componentPick: (QuickPickItem & { item?: any; type?: string })[] =
                                        components.map((item) => ({
                                            label: item.metadata.displayName,
                                            item: { data: item, type: "component" },
                                        }));
