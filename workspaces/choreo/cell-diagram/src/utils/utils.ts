...
import { CMDependency as Connection, Project } from "@wso2-enterprise/ballerina-languageclient";
        ...
        if (targetComponent) {
            ...
            for (const serviceId in component.services) {
                if (Object.prototype.hasOwnProperty.call(component.services, serviceId)) {
                    const service = component.services.get(serviceId);
...
export function getMetadataFromConnection(connection: Connection): ConnectionMetadata | null {
    ...
    if (ids.length == 4) {
        return {
            type: connection.type === "service" ? ConnectionType.HTTP : ConnectionType.GRPC,
            organization: ids[0],
            project: ids[1],
            component: ids[2],
            service: ids[3],
