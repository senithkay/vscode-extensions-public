...
import { CommandIds, type ComponentKind, type ICreateComponentParams, type Organization, type Project } from "@wso2-enterprise/wso2-platform-core";
					...
					if (userInfo) {
						...
						if (params?.componentFsPath) {
							const matchingComponent = contextStore
								.getState()
								.state?.components?.filter((item) => getNormalizedPath(item.componentFsPath) === getNormalizedPath(params?.componentFsPath));
							if (matchingComponent?.length === 0) {
								// create a new component
								window
									.showInformationMessage(`No ${extensionName} component found in this directory. Do you want to create one?`, "Proceed")
									.then((res) => {
										if (res === "Proceed") {
