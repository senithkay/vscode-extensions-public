...
export const biServerRunStateChanged: NotificationType<BiServerRunStatus> = {method: `biServerRunStateChanged`};
export type BiServerRunStatus = 'Running' | 'Stopped';

export class RuntimeServicesWebview {
    public static currentPanel: RuntimeServicesWebview | undefined;
    public static readonly viewType = 'ballerina-integrator.runtime-services-panel';
        ...
        this._panel?.onDidChangeViewState(() => {
            if (this._panel?.active || this._panel?.visible) {
                RPCLayer._messenger.sendNotification(
                    biServerRunStateChanged,
                    { type: 'webview', webviewType: 'ballerina-integrator.runtime-services-panel' },
                    StateMachine.context().biRunning ? 'Running' : 'Stopped');
            }
        });
    ...
    private static createWebview(): vscode.WebviewPanel {
        const panel = vscode.window.createWebviewPanel(
            ...
            {
                enableScripts: true,
                localResourceRoots: [Uri.file(path.join(ballerinaExtInstance.context.extensionPath, 'resources'))],
