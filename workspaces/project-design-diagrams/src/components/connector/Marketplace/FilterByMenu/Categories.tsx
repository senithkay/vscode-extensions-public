...
function Categories(props: CategoriesProps) {
    ...
    const getLabel = (text: string, styleClass?: any) => {
        return (
            <Typography variant="body1" classes={styleClass ? styleClass : { root: classes.expandaleFilterByLabel }}>
                {text}
            </Typography>
        );
    };
    ...
    return (
        <>
            ...
            >
                ...
                >
                    {connectorCategories.categories.slice(0, noOfCategoriesDisplayed).map((menuItem) => (
                        <TreeItem
                            key={menuItem.name}
                ...
                {connectorCategories.categories.length > noOfCategoriesDisplayed ? (
                    <Button onClick={onSeeMoreClick} className={classes.seeMoreButton} color="primary" size="small" startIcon={<ExpandMore />}>
                        See More
                    </Button>
                ) : (
                    <Button onClick={onSeeLessClick} className={classes.seeMoreButton} color="primary" size="small" startIcon={<ExpandLess />}>
                        See Less
                    </Button>
                )}
            </TreeItem>
