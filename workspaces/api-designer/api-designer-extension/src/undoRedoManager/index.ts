...
export class UndoRedoManager {
    ...
    public undo() {
        ...
        if (undoSourceStack && undoSourceStack.length) {
            ...
            const lastsource = undoSourceStack.pop();
            if (lastsource) {
    ...
    public redo() {
        const redoStackForPath = this.redoStack.get(this.path);
        const undoSourceStack = this.undoStack.get(this.path);
        if (redoStackForPath && redoStackForPath.length && undoSourceStack) {
            ...
            const lastUndoSource = redoStackForPath.pop();
            if (lastUndoSource) {
