...
const StyledHandle = styled.div`
    ...
    height: 10px;
    background:
        linear-gradient(135deg, transparent 60%, var(--dropdown-border) 60%, transparent 70%),
    ...
    const handleMouseMove = useCallback(
        (event: MouseEvent) => {
            if (isResizing.current && editorRef.current) {
                editorRef.current.style.height = `${event.clientY - editorRef.current.offsetTop}px`;
            }
        },
