...
export const CheckBox = ({ label, value, checked, sx, onChange }: CheckBoxProps) => {
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        onChange(e.target.checked);
    };

    return (
        <StyledCheckBox key={`checkbox-${value}`} sx={sx} value={value} checked={checked} onClick={handleChange}>
            {label}
        </StyledCheckBox>
    );
};
