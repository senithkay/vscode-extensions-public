...
import { FunctionIcon, AddIcon } from "../../../../../../assets/icons";
...
import { QueryParam, ReturnType } from "../../../Overlay/Elements/DropDown/ApiConfigureWizard/types";
...
export function FunctionConfigForm(props: FunctionConfigFormProps) {
    ...
    const { props: {syntaxTree}} = useDiagramContext();
    const existingFunctionNames = (syntaxTree as any).members.map((member: any)=>member.functionName.value);
    const disableSaveBtn = !!!functionName || existingFunctionNames.includes(functionName);

    const handleOnSave = () => {
        const parametersStr = parameters.map((item)=>`${item.type} ${item.name}`).join(',');
        const returnType = returnTypes.map((item)=>`${item.type}${item.isOptional ? '?' : ''}`).join('|');
        const returnTypeStr = returnType ? `returns ${returnType}` : '';
        const modification = createFunctionSignature(functionName, parametersStr, returnTypeStr , {line: targetPosition.startLine, column: 0});
        onSave([modification])
    };
        ...
        <FormControl data-testid="log-form" className={formClasses.wizardFormControl}>
            <div className={formClasses.formTitleWrapper}>
                <div className={formClasses.mainTitleWrapper}>
                    ...
                    <Typography variant="h4">
                        <Box paddingLeft={2} paddingY={2}>Function</Box>
                    </Typography>
                </div>
            </div>
            ...
            <div className={formClasses.sectionSeparator}>
                <Section title={'Function Name'}>
                    <FormTextInput
                        ...
                        customProps={{  isErrored: existingFunctionNames.includes(functionName)}}
            ...
            <div className={formClasses.sectionSeparator}>
                <Section title={'Parameters'}>
                    {parameters.map(param=><FunctionParamItem queryParam={param} onDelete={(param: QueryParam)=> setParameters(parameters.filter(item=>item.id!==param.id))}/>)}
                    {addingNewParam ? (
                        ...
                        <span onClick={()=>setAddingNewParam(true)} className={formClasses.addPropertyBtn}>
                            <AddIcon/>
                            <p>Add parameter</p>
                        </span>
                    )}
            ...
            <div className={formClasses.sectionSeparator}>
                <Section title={'Return Type'}>
                    {returnTypes.map(returnType=><ReturnTypeItem returnType={returnType} onDelete={(item: ReturnType)=> setReturnTypes(returnTypes.filter(returnType=>returnType.id!==item.id))}/>)}
                    {addingNewReturnType ? (
                        ...
                        <span onClick={()=>setAddingNewReturnType(true)} className={formClasses.addPropertyBtn}>
                            <AddIcon/>
                            <p>Add return type</p>
                        </span>
                    )}
            ...
            <div className={formClasses.wizardBtnHolder}>
                ...
                <PrimaryButton text="Save" disabled={disableSaveBtn} fullWidth={false} onClick={handleOnSave}
                />
            </div>
