...
export default function CreateDraftForm(props: OperationDropdownProps) {
    ...
    const validateField = (field: string, isInvalid: boolean): void => {
        ...
        for (const recordField of formFields.find(formField => formField.name === "message")?.fields) {
    ...
    const getFormFieldComponent = (component: string): any => {
        const field = formFields.find(formField => formField.name === "message")?.fields.find(inputField => inputField.name === component);
        if (!field || field?.hide) {
            return null;
        }
        // generate component
    ...
    const userIdProp: FormElementProps = {
        model: formFields.find(field => field.name === "userId"),
        index: 1,
        customProps: {
            validate: validateField,
            statementType: formFields.find(field => field.name === "userId").type
        }
    };

    const draftIdField = formFields.find(field => field.name === "draftId");
    const draftIdProp: FormElementProps = {
        model: draftIdField,
        index: 1,
        customProps: {
            validate: validateField,
            statementType: draftIdField?.type
        }
    };
        ...
        <div>
            <div className={classes.inputWrapper}>
                <div className={classes.groupedForm}>
                    {draftIdField && <ExpressionEditor {...draftIdProp} />}
