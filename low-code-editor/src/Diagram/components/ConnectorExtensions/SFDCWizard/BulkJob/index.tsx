...
import {
    ...
    createObjectDeclaration, createRemoteServiceCall
...
export function BulkJob(props: WizardProps) {
        ...
        if (targetPosition) {
            ...
            if (!connectorConfig.isExistingConnection) {
                if ((connectorConfig.name !== undefined) && (connectorConfig.name !== '') &&
                    ...
                    connectorConfig.name = genVariableName("bulkJob", getAllVariables(stSymbolInfo));
                    const bulkClientName = genVariableName("bulkClient", getAllVariables(stSymbolInfo));
                    const bulkClientInvocation: STModification = createRemoteServiceCall(
                        (connector.module + ":BulkClient"),
                        bulkClientName,
                    ...
                    const addBulkJobInit: STModification = createCheckedRemoteServiceCall(
                        connector.module + ":" + connector.name,
                        connectorConfig.name,
                        bulkClientName,
                        "creatJob",
                        ["\"" + connectorConfig.connectorInit[5].value + "\", " +
                            connectorConfig.connectorInit[6].value + ", " +
                            "\"" + connectorConfig.connectorInit[7].value + "\""],
                        targetPosition
                    );
                    modifications.push(bulkClientInvocation);
                    modifications.push(addBulkJobInit);
                } else {
                    const bulkClientName = genVariableName("bulkClient",
                        getAllVariables(stSymbolInfo));
                    const addConnectorInit: STModification = createObjectDeclaration(
                        (connector.module + ":BulkClient"),
                        bulkClientName,
                        ["{" + connectorInitFormFields[4].fields[0].name + ":" +
                            connectorInitFormFields[4].fields[0].value + ", clientConfig: {}}"],
                        targetPosition
                    );
                    const addBulkJobInit: STModification = createCheckedRemoteServiceCall(
                        (connector.module + ":" + connector.name),
