...
export function CreateConnectorForm(props: CreateConnectorFormProps) {
    ...
    const connectorInitFormFields: FormField[] = connectorConfig && connectorConfig.connectorInit &&
    connectorConfig.connectorInit.length > 0 ? connectorConfig.connectorInit : actions.get("init") ?
        actions.get("init") : actions.get("__init");
    const classes = useStyles();

    // Creating non existing form fields
    const baseUrlField: FormField = {name: "baseUrl", type: "string", isParam: true, value: ""};
    connectorInitFormFields.push({name: "salesforceConfig", type: "record", isParam: true, fields: [baseUrlField]});
    connectorInitFormFields.push({name: "operation", type: "string", isParam: true, value: ""});
    connectorInitFormFields.push({name: "sob", type: "string", isParam: true, value: ""});
    connectorInitFormFields.push({name: "jobType", type: "string", isParam: true, value: ""});
    ...
    const [isReqFieldsFilled, setIsReqFieldsFilled] = useState(false);
    const [isCustomBaseValid, setIsCustomBaseValid] = useState(false);
    const [isSObValid, setIsSObValid] = useState(false);
    ...
    const isRequiredFieldFilled = () => {
        return ((connectorInitFormFields[5].value !== "") && isSObValid &&
            (connectorInitFormFields[7].value !== "") &&
            ((connectorConfig.subExitingConnection !== undefined && connectorConfig.subExitingConnection !== "") ||
                isCustomBaseValid));
    };
    ...
    const onBaseUrlChange = (url: string) => {
        ...
        setIsReqFieldsFilled(isRequiredFieldFilled);
        setIsBaseClientSelected(false);
    };

    const onSobjChange = (url: string) => {
        setIsReqFieldsFilled(isRequiredFieldFilled);
    };

    const handleBulkOperationChange = (selectedOperation: string) => {
        connectorInitFormFields[5].value = selectedOperation;
        setIsReqFieldsFilled(isRequiredFieldFilled);
    };

    const handleJobTypeChange = (selectedJobType: string) => {
        connectorInitFormFields[7].value = selectedJobType;
        setIsReqFieldsFilled(isRequiredFieldFilled);
    };
    ...
    const handleBaseClientChange = (client: string) => {
        ...
        connectorInitFormFields[4].fields[0].value = "";
    ...
    // TODO Fix useEffect
    React.useEffect(() => {
        // if model has value make button enabled
        setIsReqFieldsFilled(isRequiredFieldFilled);
        setDefineConnector(!((baseClients.length > 0) && ((connectorInitFormFields[4].fields[0].value === "") ||
                (connectorInitFormFields[4].fields[0].value === undefined))));
    });
    ...
    const validateBaseUrl = (field: string, isInvalid: boolean) => {
        setIsCustomBaseValid(!isInvalid);
    };

    const baseUrlExpElementProps: FormElementProps = {
        model: connectorInitFormFields[4].fields[0],
    ...
    const formTitle: string = ((connectorConfig.isNewConnector !== undefined) && !connectorConfig.isNewConnector) ?
        "Bulk Job Connection" : titlePrefix + " Bulk Job Connection";

    const validateSOb = (field: string, isInvalid: boolean) => {
        setIsSObValid(!isInvalid);
    };

    const sObExpElementProps: FormElementProps = {
                ...
                >
                        ...
                        ((connectorConfig.isNewConnector === undefined) || connectorConfig.isNewConnector) ?
                            (
                                <>
                                    ...
                                    />
                                    <ExpressionEditor {...sObExpElementProps} />
                                    <SelectDropdownWithButton
                                        customProps={{
                                            disableCreateNew: true, values: ["insert", "delete",
                                                "update", "upsert", "query"]
                                        }}
                                        label="Bulk Operation"
                                        onChange={handleBulkOperationChange}
                                        placeholder="Select"
                                        defaultValue={connectorInitFormFields[5].value}
                                    />
                                    <SelectDropdownWithButton
                                        customProps={{disableCreateNew: true, values: ["JSON", "XML", "CSV"]}}
                                        label="Job Type"
                                        onChange={handleJobTypeChange}
                                        placeholder="Select"
                                        defaultValue={connectorInitFormFields[7].value}
                                    />
                    ...
                    <div className={classes.wizardBtnHolder}>
                        ...
                        <PrimaryButton
                            text="Next"
                            disabled={
                                !(((isReqFieldsFilled && nameState.isNameProvided && nameState.isValidName) ||
                                    isExistingClientSelected) && ((actionNameSelect && actionNameSelect !== "")))
                            }
                            fullWidth={false}
