...
export function getPerformance(model: STNode){
    let performance: any;
    if (model) {
        const findPerformanceVisitor = new InvocationPerformanceVisitor();
        traversNode(model, findPerformanceVisitor);
        const performanceNode = findPerformanceVisitor.getPerformanceNode();
