...
import {
  ConnectionType,
  OauthConnectButton,
} from "../../../../components/OauthConnectButton";
import {
  CONNECTOR_CLOSED,
...
export function ConnectorForm(props: ConnectorConfigWizardProps) {
  ...
  const connectorName = connectorInfo.displayName || connectorInfo.package.name;
  ...
  const [selectedActiveConnection, setSelectedActiveConnection] = useState<
    ConnectionDetails
  >();
  ...
  const updateConfigErrorMessage = intl.formatMessage({
    ...
    defaultMessage:
      "An error occurred while updating the connection configuration. Please try again.",
  });
  ...
  const createConfigErrorMessage = intl.formatMessage({
    ...
    defaultMessage:
      "An error occurred while saving the connection configuration. Please try again.",
  });
  const configTokenErrorMessage = intl.formatMessage({
    id:
      "lowcode.develop.connectorForms.manualConnection.configAccessToken.error",
  ...
  useEffect(() => {
    if (isNewConnection && isOauthConnector) {
      ...
      connectorInfo.package.organization === defaultOrgs.WSO2
    ) {
      ...
      if (connectorConfig?.connectorInit?.length > 0) {
        (async () => {
          ...
          const activeConnection = getOauthConnectionFromFormField(
            connectorConfig.connectorInit[0],
            allConnections
          );
          if (activeConnection) {
  ...
  config.name =
    isNewConnectorInitWizard && !config.name
      ? genVariableName(
          getFormattedModuleName(connectorInfo.package.name) + "Endpoint",
  ...
  const onSelectExisting = (value: any) => {
    if (connectorInfo.package.organization === defaultOrgs.WSO2) {
      setIsNewConnection(true);
      setFormState(FormStates.SingleForm);
    } else {
      setConfigName(value);
  ...
  const showNotification = (
    ...
    if (action === ConnectionAction.create) {
      if (status === 200 || status === 201) {
        ...
        triggerErrorNotification(configTokenErrorMessage);
      } else {
  ...
  const handleModifyDiagram = (modifications: STModification[]) => {
  ...
  const handleClientOnSave = () => {
    ...
    if (isOauthConnector && !isManualConnection && connection) {
      let OAuthtype;
      if (
        connection.codeVariableKeys.find((field) => field.name === "tokenKey")
          ?.name
      ) {
        OAuthtype = "BearerTokenConfig";
      } else if (
        connection.codeVariableKeys.find(
          (field) => field?.name === "clientIdKey"
        )?.name
      ) {
        OAuthtype = "OAuth2RefreshTokenGrantConfig";
      }
      const connectorConfigurables = getOauthConnectionConfigurables(
        connectorInfo.displayName.toLocaleLowerCase(),
        connection,
        symbolInfo.configurables,
        OAuthtype
      );
      ...
      if (isNewConnectorInitWizard && targetPosition) {
        ...
        const addImport: STModification = createImportStatement(
          connectorInfo.package.organization,
        ...
        const addConnectorInit: STModification = createPropertyStatement(
          `${moduleName}:${connectorInfo.name} ${config.name} = ${
      ...
      if (connectorTypes.includes(connectorInfo.displayName)) {
        const selectedType = getManualConnectionTypeFromFormFields(
          config.connectorInit
        );
        const manualConnectionFormFieldValues = getManualConnectionDetailsFromFormFields(
          config.connectorInit
        );
        ...
        (async () => {
          ...
          if (isNewConnectorInitWizard && targetPosition) {
            if (formattedFieldValues.length > 0) {
              response = await createManualConnection(
                userInfo?.selectedOrgHandle,
              ...
              if (apiResponseStatus === 200 || apiResponseStatus === 201) {
                ...
                connectorConfigurables = getOauthConnectionConfigurables(
                  connectorInfo.displayName.toLocaleLowerCase(),
                  response.data,
                  symbolInfo.configurables,
                  selectedType
            ...
            manualConnectionFormFieldValues.selectedFields.forEach(
              (item: any) => {
                if (
            ...
            if (config.isConnectionNameUpdated || updatedFields.length > 0) {
              response = await updateManualConnection(
                activeConnectionId,
                userInfo?.selectedOrgHandle,
                connectorInfo.displayName,
                config?.connectionName,
              ...
              if (
                ...
                !config.isConnectionNameUpdated &&
                config.name
              ) {
                const updateConnectorInit = updatePropertyStatement(
                  `${moduleName}:${connectorInfo.name} ${config.name} = ${
    ...
    if (!isNewConnectorInitWizard) {
      if (currentActionReturnType.hasReturn) {
        ...
        const updateActionInvocation = updatePropertyStatement(
          `${currentActionReturnType.returnType} ${
        ...
        const updateActionInvocation = updatePropertyStatement(
          `${currentActionReturnType.hasError ? "check" : ""} ${config.name}->${
            config.action.name
          }(${getParams(config.action.fields).join()});`,
          model.position
        );
        modifications.push(updateActionInvocation);
      }
      ...
      if (targetPosition) {
        if (
          connectorTypes.includes(connectorInfo.displayName) &&
          !connection &&
          !isAction
        ) {
          const selectedType = getManualConnectionTypeFromFormFields(
          ...
          (async () => {
            ...
            if (apiResponseStatus === 200 || apiResponseStatus === 201) {
              ...
              if (
                config.responsePayloadMap &&
                config.responsePayloadMap.isPayloadSelected
              ) {
          ...
          if (isOauthConnector && connection) {
            ...
            const connectorConfigurablesInvoke = getOauthConnectionConfigurables(
              connectorInfo.displayName.toLocaleLowerCase(),
              connection,
              symbolInfo.configurables,
              OAuthtype
        ...
        if (currentActionReturnType.hasReturn) {
          ...
          const addActionInvocation = createPropertyStatement(
            `${currentActionReturnType.returnType} ${
              ...
              existingEndpointName || config.name
            }->${config.action.name}(${getParams(
              config.action.fields
            ).join()});`,
            targetPosition
  ...
  const onConnectionNameChange = () => {
    if (
      (isNewConnection && !isOauthConnector) ||
    ...
    if (returnType.importTypeInfo) {
      returnType.importTypeInfo?.forEach((typeInfo) => {
        ...
        const existsMod = modifications.find(
          (modification) =>
            JSON.stringify(addImport) === JSON.stringify(modification)
        );
        if (!existsMod) {
          modifications.push(addImport);
        }
        ...
        if (actionReturnType?.hasReturn) {
          const updateActionInvocation = updatePropertyStatement(
            `${actionReturnType.returnType} ${
              config.action.returnVariableName
            } = ${actionReturnType.hasError ? "check" : ""} ${config.name}->${
              config.action.name
            }(${getParams(config.action.fields).join()});`,
            model.position
          );
          ...
          <div className={wizardClasses.topTitleWrapper}>
            ...
            <div className={wizardClasses.titleWrapper}>
              <div className={wizardClasses.connectorIconWrapper}>
                {getConnectorIcon(
                  `${connectorInfo.package.name}_${connectorInfo.name}`
                )}
              </div>
              ...
              {isOauthConnector && !isManualConnection && !isAction && (
                <div
                  className={classNames(
                    wizardClasses.bottomBtnWrapper,
                    wizardClasses.bottomRadius
                  )}
                >
                  <div className={wizardClasses.fullWidth}>
                    <div className={wizardClasses.mainOauthBtnWrapper}>
                      <OauthConnectButton
                        className={classNames(
                          wizardClasses.fullWidth,
                          wizardClasses.oauthBtnWrapper
                        )}
...
function getOauthConnectionParams(
  arg0: string,
